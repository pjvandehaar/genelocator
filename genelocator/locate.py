...
class GeneLocator:
    def __init__(self, genes):
        ...
        for gene in genes:
            ...
            if not isinstance(start, int) or not isinstance(end, int):
                raise Exception("start and end must be int, unlike {!r} and {!r}".format(start, end))
            if ensg in self._gene_info:
                raise Exception("The gene {!r} appears multiple times in this genes list (as {!r} and {!r})".format(
    ...
    def at(self, chrom, pos):
        ...
        if chrom not in self._its:
            raise Exception("Unknown chromosome: {!r}".format(chrom))
        # If any genes overlap this position, return them all.
        overlapping_genes = self._its[chrom].at(pos) if hasattr(self._its[chrom], 'at') else self._its[chrom].search(
            pos)  # support intervaltree 2.x and 3.x
        if overlapping_genes:
            return [self._with_info(g.data) for g in overlapping_genes]
        ...
        if prev_gene_end is None and next_gene_start is None:
            raise Exception('The position chr{!r}:{!r} has no genes before it or after it'.format(chrom, pos))
        if next_gene_start is None:
            return [self._with_info(prev_gene_end[1])]
        if prev_gene_end is None:
            return [self._with_info(next_gene_start[1])]
        ...
        return [self._with_info(prev_gene_end[1])] if dist_to_prev_gene_end < dist_to_next_gene_start else [
            self._with_info(next_gene_start[1])]
